...
\begin{itemize}
    ...
    \item \textbf{snowflake-arctic-embed-m \cite{logo:snowflake_cortex} :} Un modèle développé par Snowflake, intégré nativement à Snowflake Cortex, produisant des vecteurs de 768 dimensions. Optimisé pour la recherche sémantique sur des textes techniques, il s'exécute directement via la fonction SQL \texttt{EMBED\_TEXT\_768()} sans aucun transfert de données hors de l'environnement Snowflake.
...
L'approche RAG appliquée aux tickets a été explorée dans plusieurs travaux récents \cite{logo:advanced_rag}. Elle permet d'ancrer les prédictions sur des tickets historiques similaires, améliorant la cohérence et la traçabilité des classifications. Des variantes intègrent un re-ranking hybride combinant similarité sémantique et correspondance lexicale, afin de corriger les biais de la seule similarité cosinus sur les termes techniques discriminants.
...
\begin{tabularx}{\textwidth}{|l|X|X|X|}
  \hline
  \textbf{Critère} & \textbf{Snowflake Cortex} & \textbf{APIs externes (OpenAI, Anthropic)} & \textbf{Déploiement local (Ollama, HuggingFace)} \\
  \hline
  ...
  \hline
  Latence & Faible (réseau interne) & Variable (réseau public) & Faible si GPU, lente sur CPU \\
  \hline
  Coût & Crédits Snowflake & Facturation à l'appel (tokens) & Gratuit (hors infrastructure GPU) \\
  \hline
  Intégration SQL & Native & Non & Non \\
  \hline
  Conformité & Élevée & Dépend des CGU du fournisseur & Maximale \\
  \hline
  \rowcolor{headerblue} Retenu & \checkmark & & \\
  \hline
\end{tabularx}
...
\textbf{Snowflake Cortex \cite{logo:snowflake_cortex}} pour l'ensemble des fonctions d'IA. Il permet d'exécuter les modèles \texttt{mistral-large2} \cite{logo:mistral} pour la summarization et l'extraction de mots clés, \texttt{snowflake-arctic-embed-m} pour les embeddings 768 dimensions, et \texttt{llama3.3-70b} \cite{logo:llama} pour la classification few-shot et la génération de résumés correctifs, le tout directement depuis SQL sans aucun transfert de données hors de Snowflake.
