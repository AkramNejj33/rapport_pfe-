...
\begin{itemize}
    ...
    \item La quatrième famille, qualifiée d'hybride, associe une recherche par similarité vectorielle à un raisonnement par modèle de langage, selon le principe du Retrieval-Augmented Generation (RAG) \cite{logo:rag}. Son intérêt réside dans la complémentarité entre la rapidité de la recherche par plus proches voisins sémantiques et la finesse du raisonnement du LLM, enrichi de façon dynamique par les exemples historiques les plus pertinents retrouvés dans la base vectorielle.
\end{itemize}
...
Le moteur de classification s'appuie sur un pipeline RAG (Retrieval-Augmented Generation) implémenté entièrement sur Snowflake Cortex. Il combine une recherche par similarité vectorielle (embeddings \texttt{snowflake-arctic-embed-m} 768D), un re-ranking hybride sémantique et mots clés, et une classification finale par \texttt{llama3.3-70b} via few-shot prompting avec chain-of-thought. Ce pipeline produit, pour chaque ticket, une prédiction du type d'incident, une prédiction de la résolution probable et un résumé correctif court.
...
\begin{itemize}
    \item Le premier consiste à bâtir une architecture en médaillon fiable sur Snowflake, pilotée par dbt pour la couche de transformation, et à implémenter le pipeline de classification RAG entièrement sur Snowflake Cortex, sans infrastructure externe.
